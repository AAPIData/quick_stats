...
# The issue is here, you have only calculated 1 percentage, the percentage LEP for the STATE, see lines # 315,516,317 to see how I created the state AA and PI versions, then see how they are expressed in the # toolip, on lines 322 and 323, Also I would consider moving the maps into each topic section, so there should be a "visualize data" tab for EDU and a separate one for LEP
state_LEP_map <- state_LEP_map %>% mutate(
  pct_lep = round(`Total population_pct_lep`*100,2))

x <- c("{point.name}:", "Asian Alone:","NHPI Alone:")
title<-"Percent of LEP"
...
data(usgeojson)
highchart() %>%
  hc_title(text = "LEP", align = "center") %>%
  hc_subtitle(text = "Source: 2011-2015 ACS using Asian/NHPI Alone", align = "center") %>%
  hc_chart(backgroundColor = "transparent") %>%
  hc_add_series_map(usgeojson, state_LEP_map, name = "Limited English Proficiency",value = "pct_lep", joinBy = c("name", "geodisplaylabel"),
                    borderColor= "transparent") %>%
  hc_tooltip(pointFormat = tooltip_table(x,y,title), useHTML=T,  headerFormat= "")%>%
  hc_add_theme(hc_theme_538()) %>%
  hc_colorAxis(stops = color_stops(5),labels= list(format = "{value}%"),showInLegend=T) %>%
